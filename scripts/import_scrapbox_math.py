...
DATE = '2026-09-20'
CACHE = ROOT / 'inbox/scrapbox' / DATE / 'math-full'
MANIFEST = ROOT / 'math/sources/manifest.json'
...
class Renderer:
    ...
    def link(self, title):
        if title in PAGES:
            p = PAGES[title]
            path = ROOT / 'math' / p['group'] / (p['slug'] + '.md')
...
def build_page(data, spec, catalog, assets):
    path = ROOT / 'math' / spec['group'] / (spec['slug'] + '.md')
    raw_path = ROOT / 'math/sources' / (spec['slug'] + '.txt')
    title = data['title']
    ...
    note = [
        ...
        f'[数学の入口](../README.md) · [当時の本文](#original) · [AIによる補足](#ai-notes) · [原文テキスト]({relative(raw_path, path)}) · [Scrapbox]({source_url(title)})', '',
    ...
    note += ['<!-- END IMPORTED BODY -->', '', '<a id="ai-notes"></a>', '', f'## AIによる補足・訂正（{DATE}）', '',
             ...
             '## つながる移植ノート', '']
    related = [t for t in data.get('links', []) if t in PAGES and t != title]
    if related:
...
def build(overwrite=False):
    ...
    for group, name in GROUPS.items():
        path = ROOT / 'math' / group / 'README.md'
    ...
    manifest = {
        'imported': DATE, 'scope': '45 overview math pages and 19 explicitly selected related pages',
        ...
        'indexes': {f'math/{g}/README.md': sha((ROOT/f'math/{g}/README.md').read_bytes()) for g in GROUPS},
    }
